...
ggplot(flight_data[arr_delay >= 20], aes(format(time_hour, "%B %Y"))) +
  geom_bar(stat = 'count') +
...
weather <- weather[, .(mean_wind_speed = mean(wind_speed, na.rm = T)), by = .(date = as.Date(time_hour))]

flight_data$date <- as.Date(flight_data$time_hour)

all <- merge(flight_data, weather, by.x = 'date', by.y = 'date')
...
flights <- flight_data[, .(number_of_flights = .N), by = 'dest']
...
ggplot() +
  ...
  geom_point(
    data = geolocations,
    aes(lon, lat, size = number_of_flights),
    alpha = 0.7
  ) +
